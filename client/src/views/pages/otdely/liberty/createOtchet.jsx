...
import PercentUI from '../components/UI/percentUI'

export default function AddLibertyOtchet() {

    const [inputValues, setInputValues] = useState({})
    const [inputValues2, setInputValues2] = useState({})
    const [responseData, setResponseData] = useState([])
    const [dateValue, setDateValue] = useState('');
    const otdelendpoint = '/liberty/otchet'

    const createOtchet = async () => await api.post(`${otdelendpoint}/create`)
        ...
        try {
            const updatedDataPromises = responseData.flatMap(elem =>
                elem.otchet.map(item => {
                    ...
                    return api.patch(`${otdelendpoint}/update-otchet/${item._id}`, updatedData);
                })
            );
            ...
            const updatedDataPromises2 = responseData.flatMap(elem =>
                elem.itog.flatMap(elem => {
                    ...
                    const updatedData2 = {
                        ...
                        otelRuk: allraznica.toString(),
                        itogs: parseFloat(itogSum) - parseFloat(itogs),
                    };

                    return api.patch(`${otdelendpoint}/update-itog/${elem._id}`, updatedData2);
                })
            );
            ...
            responseData.map(elem => {
                elem.itog.forEach(elem => {
                    updatedInputValues2[elem._id] = {
                        ...
                        otelRuk: inputValues2[elem._id]?.otelRuk || elem.otelRuk,
    ...
    const fetchData = async () => {
        const response = await api.get(`${otdelendpoint}/data`)
    ...
    const [selectedTeam, setSelectedTeam] = useState('liberty')

    const fetchDataBuyer = async () => {
        try {
            if (selectedTeam === 'liberty') {
                const response = await api.get('/liberty/manager/data');
                setData(response.data);
            } else if (selectedTeam === 'monaco') {
                const response = await api.get('/monaco/manager/data');
                setData(response.data);
            } else if (selectedTeam === 'leader') {
                const response = await api.get('/leader/manager/data');
                setData(response.data);
            } else if (selectedTeam === 'turan') {
                const response = await api.get('/turan/manager/data');
                setData(response.data);
            }
        } catch (error) {
            console.error(error);
    ...
    const addSoloTable = async (id) => {
        setLoading4(false)
        try {
            await api.post('/liberty/otchet/insert', { id });
            setWait(false)
            fetchData()
        } catch (error) {
    ...
    const handleSend = async () => {
        ...
        try {
            setLoading3(false);
            await api.post('/liberty/otchet/add-data', data);
            setWait(false)
            fetchData()
        } catch (error) {
    ...
    const fetchLogist = async () => {
        const response = await api.get('/liberty/admin/data')
        const response3 = await api.get('/adminlogist')
        const filtered = response3.data.filter(item => item.team === 'Liberty')
        setLogist([...response.data, ...filtered])
    }
            ...
            {wait ? (<div className='mx-auto '>
                ...
                <div className='flex gap-5'>
                    {
                        responseData.length ? (<div className='flex justify-center items-center mb-5 gap-2'>
                            ...
                            <select
                                ...
                                className="border px-4 py-2 lg:text-[12px] text-[5px] rounded-md bg-white text-gray-800 shadow-md"
                            >
                ...
                <div className="overflow-x-auto flex flex-col items-center justify-center">
                    {responseData.map((elem) => {
                        return (
                            <div>
                                <table className="bg-white min-w-full text-center border border-collapse">
                                    {responseData.length ? (<thead className="bg-gradient-to-r from-pink-400 to-red-500 lg:text-[12px] text-[4px] text-white">
                                        <tr>
                                                    ...
                                                    <tr>
                                                        ...
                                                        <td className="border lg:py-1 lg:px-2" rowSpan="2">
                                                            ...
                                                            >
                                                                ...
                                                                <option value={4}>х% логист</option>
                                                                <option value={5}>х% онлайн</option>
                                                            </select>
                                                        </td>
                                                        ...
                                                        <td className="border lg:py-1 lg:px-2">
                                                            <input
                                                                ...
                                                                onChange={(e) => {
                                                                    ...
                                                                    const indexPersent2 = (indexPersent100 * 0.34).toFixed(0);
                                                                    const indexPersent3 = (indexPersent100 * 0.33).toFixed(0);
                                                                    const indexPersent4 = (indexPersent100 * 0.33).toFixed(0)
                                ...
                                </table>
                                {
                                    responseData.length ? (loading4 ? (<button
                                        onClick={() => addSoloTable(elem._id)}
                                        className='lg:ml-3 mt-5 mb-5 ml-7 bg-gradient-to-r from-pink-400 to-red-500 hover:bg-pink-500 text-white lg:py-2 lg:px-6 p-1 rounded-lg shadow-lg transition-all duration-300 ease-in-out text-[10px] transform hover:scale-105 focus:outline-none'
                                    >добавить слот
                                    </button>) : (<button
                                        className='lg:ml-3 mt-5 mb-5 bg-gradient-to-r from-pink-400 to-red-500 hover:bg-pink-500 text-white lg:py-2 lg:px-6 p-1 rounded-lg shadow-lg transition-all duration-300 ease-in-out text-[10px] transform hover:scale-105 focus:outline-none'
                    ...
                    {responseData.length ? (<button
                        className="lg:fixed lg:right-3 lg:bottom-1 mt-5 mb-5 bg-gradient-to-r from-pink-400 to-red-500 hover:bg-pink-500 text-white py-2 px-6 rounded-lg shadow-lg transition-all duration-300 ease-in-out text-[10px] transform hover:scale-105 focus:outline-none"
                        onClick={handleSubmit}
                    >
                        {loading ? 'Сохранить' : <LoadingAnimate />}
                    </button>) : ''}
                    <table className="mx-auto bg-white lg:min-w-full w-[300px]  lg:text-[12px] text-[5px] text-center border">
                        {responseData.length ? (<thead className="bg-gradient-to-r from-pink-400 to-red-500 hover:bg-pink-500 text-white">
                            <tr>
                                ...
                                return elem.itog.map((item, index) => {
                                    ...
                                    return (
                                        <tbody key={item._id} className='lg:text-[12px] text-[5px]'>
                                            ...
                                            <tr>
                                                <td colSpan='2' className='bg-gradient-to-r from-pink-400 to-red-500 hover:bg-pink-500 text-white'>итоги</td>
                                            </tr>
                                            <tr>
                                            ...
                                            <tr>
                                                ...
                                                <td className='border '>{item.percentRuk}</td>
                                            </tr>
                                            <tr>
                                                <td colSpan='2' className='bg-gradient-to-r from-pink-400 to-red-500 hover:bg-pink-500 font-semibold text-white'>общий итог: {item.itogs} </td>
                                            </tr>
                                        </tbody>
                                    )
                                })
...

                    {
                        responseData.length ? (loading3 ?
                            <button
                                onClick={handleSend}
                                className="mt-5 mb-5 bg-gradient-to-r from-pink-400 to-red-500 hover:bg-pink-500 text-white py-2 px-6 rounded-lg shadow-lg transition-all duration-300 ease-in-out text-[10px] transform hover:scale-105 focus:outline-none"
                            >
                                Отправить отчет
                            </button>
                            : <button
