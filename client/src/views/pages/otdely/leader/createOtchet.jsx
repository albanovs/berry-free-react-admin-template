...
export default function AddLeaderOtchet() {
    ...
    const otdelendpoint = '/leader/otchet'
    ...
    const [selectedTeam, setSelectedTeam] = useState('leader')
    ...
    const addSoloTable = async (id) => {
        setLoading4(false)
        try {
            await api.post('/leader/otchet/insert', { id });
            setWait(false)
            fetchData()
        } catch (error) {
    ...
    const handleSend = async () => {
        ...
        try {
            setLoading3(false);
            await api.post('/leader/otchet/add-data', data);
            setWait(false)
            fetchData()
        } catch (error) {
    ...
    const fetchLogist = async () => {
        const response = await api.get('/leader/admin/data')
        const response3 = await api.get('/adminlogist')
        const filtered = response3.data.filter(item => item.team === 'Лидер')
