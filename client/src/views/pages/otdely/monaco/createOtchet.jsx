...
export default function AddMonacoOtchet() {

    const [inputValues, setInputValues] = useState({})
    const [inputValues2, setInputValues2] = useState({})
    const [responseData, setResponseData] = useState([])
    const [dateValue, setDateValue] = useState('');
    const otdelendpoint = '/monaco/otchet'
    ...
    const [selectedTeam, setSelectedTeam] = useState('monaco')
    ...
    const addSoloTable = async (id) => {
        setLoading4(false)
        try {
            await api.post('/monaco/otchet/insert', { id });
            setWait(false)
            fetchData()
        } catch (error) {
    ...
    const handleSend = async () => {
        ...
        try {
            setLoading3(false);
            await api.post('/monaco/otchet/add-data', data);
            setWait(false)
            fetchData()
        } catch (error) {
    ...
    const fetchLogist = async () => {
        const response = await api.get('/monaco/admin/data')
        const response3 = await api.get('/adminlogist')
        const filtered = response3.data.filter(item => item.team === 'Монако')
