...
export default function AddTuranOtchet() {

    const [inputValues, setInputValues] = useState({})
    const [inputValues2, setInputValues2] = useState({})
    const [responseData, setResponseData] = useState([])
    const [dateValue, setDateValue] = useState('');
    const otdelendpoint = '/turan/otchet'
    ...
    const [selectedTeam, setSelectedTeam] = useState('turan')
    ...
    const addSoloTable = async (id) => {
        setLoading4(false)
        try {
            await api.post('/turan/otchet/insert', { id });
            setWait(false)
            fetchData()
        } catch (error) {
    ...
    const handleSend = async () => {
        ...
        try {
            setLoading3(false);
            await api.post('/turan/otchet/add-data', data);
            setWait(false)
            fetchData()
        } catch (error) {
    ...
    const fetchLogist = async () => {
        const response = await api.get('/turan/admin/data')
        const response3 = await api.get('/adminlogist')
        const filtered = response3.data.filter(item => item.team === 'Туран')
