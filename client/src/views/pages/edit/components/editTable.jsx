...
export default function Simcard({ datas, api, name }) {
        ...
        try {
            const updatedDataPromises = datas.flatMap(item =>
                item.slot.map(elem => {
                    ...
                    return fetchApi.patch(`/${api}/update/` + elem._id, updateData);
                })
            );
    ...
    const curatorChange = async (id) => {
        try {
            ...
            await fetchApi.patch(`/${api}/curator/${id}`, datas);
            window.location.reload()
        } catch (error) {
            console.log(error);
        }
    ...
    const confirmDelete = async () => {
        try {
            await fetchApi.delete(`/${api}/delete-manager/${selectedCurator}`);
            setShowModal(false);
            window.location.reload()
        } catch (error) {
    ...
    const addSimSlot = async () => {
        try {
            await fetchApi.post(`/${api}/create`, {
                curator: curator,
            });
            window.location.reload();
        } catch (error) {
    ...
    const addSlots = async (id) => {
        try {
            await fetchApi.post(`/${api}/add-slot`, { id });
            window.location.reload();
        } catch (error) {
            console.error(error);
        }
    ...
    const handleInputChangeDate = async (event, elemId, field) => {
        ...
        try {
            await fetchApi.patch(`/${api}/updatedays`, {
                itemId: elemId,
                field: field,
                value: formattedDate,
                days_since_verification: daysDiff.toString(),
        ...
        <>
            <table className="table-auto w-full border-collapse border border-gray-300 rounded-lg">
                <thead className="bg-gray-100">
                    <tr>
                        <th className="border border-gray-300 px-4 py-2 text-left font-medium text-gray-700">Общая сим-карта</th>
                        <th className="border border-gray-300 px-4 py-2 text-left font-medium text-gray-700">Свободные сим-карты</th>
                        <th className="border border-gray-300 px-4 py-2 text-left font-medium text-gray-700">Использованные сим-карты</th>
                    </tr>
                </thead>
                <tbody>
                    <tr className="bg-white hover:bg-gray-50">
                        <td className="border border-gray-300 px-4 py-2 text-gray-600">{sumSimWithNumber}</td>
                        <td className="border border-gray-300 px-4 py-2 text-gray-600">{sumSimWithStatus}</td>
                        <td className="border border-gray-300 px-4 py-2 text-gray-600">{sumSimWithNumber - sumSimWithStatus}</td>
                    </tr>
                </tbody>
            </table>

            {
                loading ? (datas.map((elem, index) => (
                    <div key={elem._id} className="mt-10">
                        <table className='min-w-full bg-white text-center border border-collapse shadow-md rounded-lg'>
                                        ...
                                        <tbody key={item._id} className={isOdd ? "bg-gray-100 text-[4px] lg:text-[10px]" : "text-[4px] lg:text-[10px]"}>
                                            <tr>
                                                <th className='border lg:py-1 lg:px-3' onClick={async () => {
                                                    try {
                                                        await fetchApi.delete(`/${api}/delete-slot/${item._id}`);
                                                        window.location.reload()
                                                    } catch (error) {
                                                ...
                                                <th className={`status border lg:py-1 lg:px-3 ${inputValues[item._id]?.status === '2' ? 'bg-emerald-200' : 'bg-amber-200'}`}>
                                                    <select
                                                        className={`outline-none ${inputValues[item._id]?.status === '2' ? 'bg-emerald-200' : 'bg-amber-200'}`}
                                                ...
                                                <th className={`border lg:py-1 lg:px-3 ${inputValues[item._id]?.status_simCard === '1' ? 'bg-emerald-200' : inputValues[item._id]?.status_simCard === '2' ? 'bg-amber-200' : inputValues[item._id]?.status_simCard === '3' ? 'bg-red-400' : 'bg-emerald-200'}`}>
                                                    <select
                                                        className={
                                                            `outline-none  ${inputValues[item._id]?.status_simCard === '1' ? 'bg-emerald-200' : inputValues[item._id]?.status_simCard === '2' ? 'bg-amber-200' : inputValues[item._id]?.status_simCard === '3' ? 'bg-red-400' : 'bg-emerald-200'}`
                                                        }
                                                ...
                                                <th className={`border lg:py-1 lg:px-3 ${inputValues[item._id]?.physical_simCard === '1' ? 'bg-emerald-200' : inputValues[item._id]?.physical_simCard === '2' ? 'bg-red-400' : inputValues[item._id]?.physical_simCard === '3' ? 'bg-amber-200' : 'bg-emerald-200'}`}>
                                                    <select className={`outline-none ${inputValues[item._id]?.physical_simCard === '1' ? 'bg-emerald-200' : inputValues[item._id]?.physical_simCard === '2' ? 'bg-red-400' : inputValues[item._id]?.physical_simCard === '3' ? 'bg-amber-200' : 'bg-emerald-200'}`}
                        ...
                        <button className='lg:ml-3 mt-5 mb-5 bg-gradient-to-r from-pink-400 to-red-500 hover:bg-blue-600 text-white lg:py-2 lg:px-6 p-1 rounded-lg shadow-lg transition-all duration-300 ease-in-out text-[10px] transform hover:scale-105 focus:outline-none' onClick={() => addSlots(elem._id)}>добавить байера</button>
                    </div>
                ))) : <div>Загрузка...</div>
            }
            {
            ...
            <div className="mb-10">
                <hr />
                <h1 className="text-center mt-5 mb-5">Добавить нового куратора</h1>
                <div className="flex items-center justify-center gap-2">
                    ...
                    <div>
                        <button
                            className='lg:ml-3 mt-5 mb-5 bg-gradient-to-r from-pink-400 to-red-500 hover:bg-blue-600 text-white lg:py-2 lg:px-6 p-1 rounded-lg shadow-lg transition-all duration-300 ease-in-out text-[10px] transform hover:scale-105 focus:outline-none'
